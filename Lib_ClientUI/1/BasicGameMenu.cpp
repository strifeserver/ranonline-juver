	...
	case MENU_QUEST_BUTTON:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				if ( dwMsg & UIMSG_LB_UP )
				{
					...
					if ( !CInnerInterface::GetInstance().IsVisibleGroup ( questui_id ) )
					{
						DWORD dwEventQuestID = CInnerInterface::GetInstance().GetEventQuestID ();
						if ( NATIVEID_NULL().dwID == dwEventQuestID )
						{
							CInnerInterface::GetInstance().ShowGroupFocus ( questui_id );
					...
					{
						CInnerInterface::GetInstance().HideGroup ( questui_id );
					}
				}
			}
		}

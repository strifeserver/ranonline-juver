...
void CLargeMapWindow::SetMapAxisInfo ( GLMapAxisInfo& sInfo, const CString& strMapName )
{
	...
	if( RANPARAM::strCountry == "cn" ){
		/*std::string strReplaced = m_strMapTexture.c_str();
		std::size_t nfound = strReplaced.rfind('.dds');
		strReplaced.erase(nfound);
		m_strMapTexture = strReplaced.c_str() + "-cn.dds";*/
		std::string m_strMapTexture = sInfo.GetMinMapTex();
		std::string foo = m_strMapTexture;
		...
		foo.replace(pos, len, "-cn.dds");

		m_strMapTexture = foo;
		//cFILE.getflag ( "MINIMAPNAME", 1, 1, foo );
		m_pMap->SetTextureName ( m_strMapTexture.c_str() );
	}else{

	...
	m_bBLOCK ( false )
{
	...
	memset ( m_pWhiteBack, 0, sizeof ( CUIControl* ) * 10 );//itemmall
}
...
//		GASSERT ( 0 && "범위를 넘어섭니다." );
	...
	if ( m_bItemShop )
	{
		...
		if ( m_InvenItem[nIndex].wCurrency == 1 )
			...
			m_pItemCur[nIndex]->SetText ( strText3.GetString() , NS_UITEXTCOLOR::GREEN_WOW );

		m_pItemNameStatic[nIndex]->SetText( ID2GAMEWORD( "ITEMSHOP_TEXTBOX", 0 ), NS_UITEXTCOLOR::YELLOW );
		m_pItemPriceStatic[nIndex]->SetText( ID2GAMEWORD( "ITEMSHOP_TEXTBOX", 1 ), NS_UITEXTCOLOR::YELLOW );
		m_pItemStockStatic[nIndex]->SetText( ID2GAMEWORD( "ITEMSHOP_TEXTBOX", 2 ), NS_UITEXTCOLOR::YELLOW );	
		...
		for ( int i = 0; i < m_nMaxColumn; i++ )
			{
				...
				m_pItemPriceStatic[i]->ClearText();
				m_pItemStockStatic[i]->ClearText();
				m_pItemNameStatic[i]->SetVisibleSingle ( FALSE ); //try
				m_pItemPriceStatic[i]->SetVisibleSingle ( FALSE );
				m_pItemStockStatic[i]->SetVisibleSingle ( FALSE );
				m_pItemPrice[i]->SetVisibleSingle ( FALSE );
				m_pItemStock[i]->SetVisibleSingle ( FALSE );
				...
				if ( pItemData )
				{
					...
					if ( bCheckItemShop() )
					{
						...
						if ( m_InvenItem[i].wCurrency == 1 )
							...
							m_pItemCur[i]->SetText ( strText3.GetString() , NS_UITEXTCOLOR::GREEN_WOW );
						m_pItemCur[i]->SetVisibleSingle ( TRUE );
						m_pItemNameStatic[i]->SetText( ID2GAMEWORD( "ITEMSHOP_TEXTBOX", 0 ), NS_UITEXTCOLOR::YELLOW );
						m_pItemPriceStatic[i]->SetText( ID2GAMEWORD( "ITEMSHOP_TEXTBOX", 1 ), NS_UITEXTCOLOR::YELLOW );
						m_pItemStockStatic[i]->SetText( ID2GAMEWORD( "ITEMSHOP_TEXTBOX", 2 ), NS_UITEXTCOLOR::YELLOW );	
	...
	void CItemSlot::SetNumber2 ( int nIndex, int nNumber )
{
	CString strNumber;
	strNumber.Format ( "+%d", nNumber );
	DWORD dwColor = NS_UITEXTCOLOR::PALEGREEN;
	m_pNumberBoxArray[nIndex]->SetOneLineText ( strNumber, dwColor );
	m_pNumberBoxArray[nIndex]->SetVisibleSingle ( TRUE );
}
...
{
	CD3DFontPar* pFont = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 8, _DEFAULT_FONT_FLAG/*_DEFAULT_FONT_FLAG*/ );

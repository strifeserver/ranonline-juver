...
void DxSkinMesh9_NORMAL::DrawMeshContainer( IDirect3DDevice9 *pd3dDevice, const CHARSETTING& sSetting )
{
				...
				if( m_dwMaxVertexBlendMatrices < NumBlend + 1)
				{
					...
					for (DWORD i = 0; i < dwNumInfl; ++i)
					{
						...
						if (iMatrixIndex != UINT_MAX)
						{
							...
							D3DXMatrixMultiply( &g_pBoneMatrices[i], &pMeshContainer->pBoneOffsetMatrices[iMatrixIndex], &matBoneMatrix_UP );
							V( pd3dDevice->SetTransform( D3DTS_WORLDMATRIX( i ), &g_pBoneMatrices[i] ) );
						}
					}
	...
	pd3dDevice->SetSamplerState( 0, D3DSAMP_MINFILTER, RANPARAM::GetTextureFilter() );
	pd3dDevice->SetSamplerState( 0, D3DSAMP_MAGFILTER, RANPARAM::GetTextureFilter() );
	pd3dDevice->SetSamplerState( 0, D3DSAMP_MAXANISOTROPY, RANPARAM::GetTextureAF() );

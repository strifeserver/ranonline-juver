...
void DxEffectRainPoint::SetProperty ( PBYTE &pProp, DWORD &dwSize, DWORD dwVer )
{
	...
	{
		CDebugSet::ErrorVersion( "DxEffectRainPoint::SetProperty", dwVer );
	}
}
...
void DxEffectRainPoint::LoadBufferSet ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	{
		CDebugSet::ErrorVersion( "DxEffectRainPoint::LoadBufferSet", SFile.GetFileName(), dwVer );
...
//	GASSERT(pd3dDevice);
	...
	{
		CDebugSet::ErrorVersion( "DxEffectRainPoint::LoadBuffer", SFile.GetFileName(), dwVer );
		DWORD dwCur = SFile.GetfTell();
		SFile.SetOffSet ( dwCur+dwBuffSize );
		return;
	}

...
void DxEffectCloth::SetProperty ( PBYTE &pProp, DWORD &dwSize, DWORD dwVer )
{
	...
	{
		CDebugSet::ErrorVersion( "DxEffectCloth::SetProperty", dwVer );
	}
...
void DxEffectCloth::LoadBufferSet ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( dwVer > VERSION )
	{
		CDebugSet::ErrorVersion( "DxEffectCloth::LoadBufferSet", SFile.GetFileName(), dwVer );
...
void DxEffectCloth::LoadBuffer ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( dwVer > VERSION )
	{
		CDebugSet::ErrorVersion( "DxEffectCloth::LoadBuffer", SFile.GetFileName(), dwVer );
		DWORD dwCur = SFile.GetfTell();
		SFile.SetOffSet ( dwCur+dwBuffSize );
		return;
	}

...
void DxEffectSpecReflect::SetProperty ( PBYTE &pProp, DWORD &dwSize, DWORD dwVer )
{
	...
	{
		CDebugSet::ErrorVersion( "DxEffectSpecReflect::SetProperty", dwVer );
	}
}
...
void DxEffectSpecReflect::LoadBufferSet ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( dwVer > VERSION )
	{
		CDebugSet::ErrorVersion( "DxEffectSpecReflect::LoadBufferSet", SFile.GetFileName(), dwVer );
...
void DxEffectSpecReflect::LoadBuffer ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( dwVer > VERSION )
	{
		CDebugSet::ErrorVersion( "DxEffectSpecReflect::LoadBuffer", SFile.GetFileName(), dwVer );
		DWORD dwCur = SFile.GetfTell();
		SFile.SetOffSet ( dwCur+dwBuffSize );
		return;
	}

...
void DxEffectSpecular2::SetProperty ( PBYTE &pProp, DWORD &dwSize, DWORD dwVer )
{
	...
	{
		CDebugSet::ErrorVersion( "DxEffectSpecular2::SetProperty", dwVer );
	}
...
void DxEffectSpecular2::LoadBufferSet ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( dwVer > VERSION )
	{
		CDebugSet::ErrorVersion( "DxEffectSpecular2::LoadBufferSet", SFile.GetFileName(), dwVer );
...
void DxEffectSpecular2::LoadBuffer ( CSerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( dwVer > VERSION )
	{
		CDebugSet::ErrorVersion( "DxEffectSpecular2::LoadBuffer", SFile.GetFileName(), dwVer );
		DWORD dwCur = SFile.GetfTell();
		SFile.SetOffSet ( dwCur+dwBuffSize );
		return;
	}

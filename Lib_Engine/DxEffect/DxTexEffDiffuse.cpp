...
void DxTexEffDiffuse::SetProperty( LPDIRECT3DDEVICEQ pd3dDevice, PBYTE &pProp )
{
	...
	m_sProp.bWhenDayOff	= pTemp->bWhenDayOff;
...
{
	// dont render color during daytime
	if ( m_sProp.bWhenDayOff && GLPeriod::GetInstance().IsOffLight() )
	{
...
void DxTexEffDiffuse::LoadPSF( LPDIRECT3DDEVICEQ pd3dDevice, CSerialFile& SFile )
{
	...
	{
		CDebugSet::ErrorVersion( "DxTexEffDiffuse::LoadPSF", SFile.GetFileName(), dwVer );

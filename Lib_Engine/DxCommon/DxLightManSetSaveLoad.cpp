...
void	DXLIGHT::LoadSet_EP9( CSerialFile &SFile )
{
	...
	m_Light.Attenuation0 = float(fatt0*0.009);
	m_Light.Attenuation1 = float(fatt1*0.009)/(m_Light.Range*0.1);		// 최대거리일 경우, (값 = 색 / 1.5f)
	m_Light.Attenuation2 = float(fatt2*0.009)/(m_Light.Range*m_Light.Range*0.1);	
...
void	DXLIGHT::LoadSet_EP9_2( CSerialFile &SFile )
{
	DWORD dwVER(0);
	DWORD dwSIZE(0);
	...
	if( dwVER == 0x0100 )
	{
		...
		{		
			pNext = new DXLIGHT();
			pNext->LoadSet_EP9_2 ( SFile );	
		}
	}
	else
	{
		CDebugSet::ErrorVersion( "DXLIGHT::LoadSet_EP9_2", SFile.GetFileName(), dwVER );
...
void DxLightMan::Load ( CSerialFile &SFile, DWORD dwLandVer, BOOL bSETLAND )
{
	...
	else
	{
		...
		if( 0x00109==dwVersion )
		{
			SFile >> m_bLighting;
			SFile >> m_bWhenDayPointOff;
			SFile >> m_bNightAndDay;

			m_LtDirectNoon.LoadSet_EP9_2 ( SFile );
			m_LtDirectNight.LoadSet_EP9_2 ( SFile );

			//could be scale vmax and vmin
			float _f;
			D3DXVECTOR3 v1;
			D3DXVECTOR3 v2;
			...
			for ( DWORD _dwLight=0; _dwLight<_dwNUM; ++_dwLight )
			{
				...
				pLightLoad->LoadSet_EP9_2 ( SFile );

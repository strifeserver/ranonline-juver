...
HRESULT GLSkillMan::SyncUpdateData ()
{
	...
	for ( int j = 0; j < MAX_CLASSSKILL; j++ )
	{
		...
		for ( int k=0; k<SKILL::MAX_LEVEL; ++k )
		{
			...
			{
				CDebugSet::ToFile ( _LOGFILE, "[%03d][%03d] ����(%d) ����Ƚ���� 0 �Դϴ�.", i, j, k );
			}

			if ( sSKILL.m_sAPPLY.sDATA_LVL[k].wTARNUM==0 )
			{
				CDebugSet::ToFile ( _LOGFILE, "[%03d][%03d] ����(%d) Ÿ�ٰ����� 0 �Դϴ�.", i, j, k );
			}
		}
		...
		for ( int nClass=0; nClass<GLCI_NUM_8CLASS; ++nClass )
		{
			if ( sSKILL.m_sLEARN.dwCLASS & nCHAR_FLAGS[nClass] )
			{
				...
				if ( sANILIST.empty() )
				{
					CDebugSet::ToFile ( _LOGFILE, "[%03d][%03d][%s] ������ '��ų���'�� �������� ���� .", i, j, sSKILL.GetName() );
					continue;
				}
				...
				{
					CDebugSet::ToFile ( _LOGFILE, "[%03d][%03d][%s] '��ų���'�� 'Ÿ������'�� ����.", i, j, sSKILL.GetName() );
				}
			}
		}
	}
